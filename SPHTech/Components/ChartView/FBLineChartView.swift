...
extension FBLineChartView{
    ...
    fileprivate func createContentScrollView(){
        let frame:CGRect = CGRect.init(x: gap, y: gap, width: self.Width-gap, height: self.Height-2*gap)
...
extension FBLineChartView{
    ...
    public func refreshUIData() {
        ...
        self.scrollView.paintPolyline(self.values, pointType: .SolidPoint, lineColor: self.crossColor, maskColor: self.maskColor)
